"""TALOS — ATTACK MODULES (36-39).

Engine modules plus menu wrappers that share one engine with the
...
* ``auto_audit``      — read-only automated audit (see
  ``src.modules.auto_audit``): scans ports/services/OS and reports
  outdated-version, default-config and weak-auth *candidates* without
  ever attacking the target.  The ``menu_auto_audit`` wrapper below makes
  it reachable from the CLI menu.
...
def menu_auto_audit(session: Session) -> None:
    """CLI wrapper — read-only automated vulnerability audit (module 39)."""
    print("  Runs one nmap service/OS scan, then flags outdated services,")
    print("  default configurations and weak-auth surfaces.")
    print("  READ-ONLY: it reports candidate findings — it never exploits,")
    print("  brute-forces or modifies the target.")
    print()
    target = _ask("Target (host / IP / CIDR)", session.target_ip or session.target_domain or "")
    if not target:
        print(fmt.tag_error("No target given; aborting the audit."))
        return
    ports = _ask("Ports (comma separated, optional)", "")
    os_detect = _ask("Include OS detection (-O, needs root) [y/n]", "n").strip().lower()
    os_detect_enabled = os_detect in ("y", "yes", "1", "true")
    if os_detect_enabled and not can_run_os_detection():
        print(fmt.tag_warn(
            "OS detection (-O) needs root privileges — it will be skipped; "
            "the scan continues normally."
        ))
    scripts = _ask("Read-only NSE scripts (default: ssh-auth-methods,ftp-anon,http-default-accounts)", "")
    try:
        timeout = float(_ask("Timeout in seconds", "600"))
    except ValueError:
        timeout = 600.0
    options: Dict[str, Any] = {
        "os_detect": os_detect_enabled,
        "nse_scripts": scripts,
        "timeout": timeout,
    }
    ...
    _run_and_report(session, AutoAuditModule, context)
